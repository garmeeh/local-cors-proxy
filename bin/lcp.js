...
var optionDefinitions = [
  ...
  { name: 'help', type: Boolean, alias: 'h', description: 'print out helpful usage information'}
];
...
try {
  ...
  if(options.help) {
    ...
    console.warn(chalk.yellow('webhookStore: ' + chalk.green('Url of you webhook store. Example: --webhookStore https://coucou.webhook.store')) )    
    console.warn(chalk.yellow('port: ' + chalk.green('The port you want forward the webhooks. Example: --port 9001')) )    
  } else {
    if (!options.webhookStore) {
      throw new Error('--webhookStore is required. example --webhookStore https://claimyourdomain.webhook.store');
    }
    if (!options.port) {
      console.log(chalk.yellow('--port option was not provided, using 9000 by default.'))
    }
